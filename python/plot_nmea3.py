...
def monitor(stdscr):
    ...
    while True:
        try:
            ...
            safe_addstr(stdscr, 3, 0, f"Latitude (GGA):      {gga_data.get('lat', 'N/A')}", max_x)
            safe_addstr(stdscr, 4, 0, f"Longitude (GGA):     {gga_data.get('lon', 'N/A')}", max_x)
            ...
            sat_rows_available = max_y - sat_start_row - 1
