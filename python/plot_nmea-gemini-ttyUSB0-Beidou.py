...
# Structure:
# {
#   'BD': {'total_sats_in_view': 0, 'satellites': {PRN: {'elev': X, 'azimuth': Y, 'snr': Z}}},
# }
# This version is specifically for a BeiDou-only module, so only 'BD' is included.
satellite_data = {
    'BD': {'total_sats_in_view': 0, 'satellites': {}}, # BeiDou
}
...
def parse_gsv_sentence(line):
    ...
    try:
        ...
        talker_id = fields[0][1:3] # e.g., 'BD' for BeiDou
        message_type = fields[0][3:] # e.g., 'GSV'

        if message_type != 'GSV':
            return None # Not a GSV message (this will discard $GPTXT, etc.)
        ...
        while True:
            ...
            if line:
                ...
                if parsed_data:
                    ...
                    # Only process if the talker_id is in our defined satellite_data (i.e., 'BD')
